...
USERS_DIRECTORY = "users"                           # Directory that will contain the resulting synthetic traffic.  Automatically created if does not exist.
SYNTHETIC_DIRECTORY = "synthetic"                   # Directory that contains the synthetic request files, one per synthetic user
...
class NumberOfRequestsDistribution(Distribution):
  ...
  def choose(self):
    ...
    if self.user_class == 'HF':
      ...
      raise Exception('The user class %s does not exist' % self.user_class)
    # From R: An alternative parametrization (often used in ecology) is by the  _mean_ 'mu', and 'size', the _dispersion parameter_, where 'prob' = 'size/(size+mu)'
...
class User(object):
  ...
  # Set the traffic model that corresponds to the user class, e.g, 'HF' + peak hour => Volume {Weibull or Gamma}, Number of requests {Neg-Binomial}, ...
    ...
    for hour, traffic_model in self.traffic_model_per_hour.iteritems():
      ...
      if number_of_requests > 0:
        ...
        if number_of_requests == 1:
          ...
          # The number of requests and inter arrival times are generated by two different distributions and here is the drawback:
          # If the number of requests is big, the IAT should be distributed in small values in order to not surpass one hour,
          # But the IATDistribution is not aware about how many requests were generated, they are two independent variables. 
          # Therefore, we have to resample the mean IATs, that summed up, would not go beyond one hour.
        ...
        # If the number of requests is big, the IAT should be distributed in small values in order to not surpass one hour,
        # But the IATDistribution is not aware about how many requests were generated, they are two independent variables. 
        # Therefore, we have to remove requests, mean IAT and arrival times, that summed up, would go beyond one hour.
        ...
        for index, time in enumerate(mean_inter_arrival_times):
          if index == 0:
            ...
            arrival_datetimes.append(datetime(self.initial_timestamp_date.year, self.initial_timestamp_date.month, self.initial_timestamp_date.day, hour) + timedelta(seconds=time))
          else:
            arrival_times.append(arrival_times[index-1] + time)
            arrival_datetimes.append(datetime(self.initial_timestamp_date.year, self.initial_timestamp_date.month, self.initial_timestamp_date.day, hour) + timedelta(seconds=arrival_times[index-1] + time))
        ...
        volumes_of_traffic = []
        # The distribution for the volume of traffic was measure for the *whole* hour. It means that each sampling from the volume_distribution
        # returns the expected volume for the whole hour. We have then to divide this volume by the number of requests in that hour to have one volume per request.
        # traffic_model.volume_distribution.rvs(size = number_of_requests) / number_of_requests
        #   - this snipt generates an array with one volume of traffic per request and divide *all at the same time* by the number of requests
        volumes_of_traffic.extend(traffic_model.volume_distribution.rvs(size = number_of_requests) / number_of_requests)
        self.request_file_sizes_per_hour[hour].extend(volumes_of_traffic)
      
  def requests(self):
    for hour in range(1, 24):
      for filesize, arrival_datetime in zip(self.request_file_sizes_per_hour[hour], self.request_arrival_datetimes_per_hour[hour]):
  ...
  def write_traffic_to_file(self):
    ...
    with open(path.join(user_syntethic_trace_path, "%s.dat" % self.uid), 'w') as user_syntethic_trace_file:
      for filesize, arrival_datetime in self.requests():
        ...
        user_syntethic_trace_file.write("%s %s %s %s\n" % (arrival_datetime.strftime("%Y-%m-%d %H:%M:%S"), self.uid, filesize, self.klass)) # 2013-08-25 00:10:58 13 30.7411159743, HF
        
  def generate_and_write_synthetic_traffic(self):
    # Numpy has the same seed for each child process, thus users sharing the same distributions get same values from rvs method
    # A way workaround this is to reseed for every process spawnned by multiprocessing
...
class UserDistribution(object):
  def __init__(self, number_of_users):
    ...
    self.mo_probability = 598340.0 / self.total_number_of_users
    self.lo_probability = 405848.0 / self.total_number_of_users    # number of users in the class 'LO' divided by the total number of users
    self.ho_probability = 484959.0 / self.total_number_of_users
    self.lf_probability = 12995.0 / self.total_number_of_users
    self.mf_probability = 12577.0 / self.total_number_of_users
    self.hf_probability = 2182.0 / self.total_number_of_users
